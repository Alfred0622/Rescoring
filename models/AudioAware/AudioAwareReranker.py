...
class AudioAwareReranker(nn.Module):
    ...
    def forward(self, audio, ilens, input_ids, attention_mask, labels, use_pos_only = False):
        ...
        if (not use_pos_only):
            ...
            bert_embedding = torch.zeros((labels.shape[0], labels.shape[1], 768).shape).to(self.device)
