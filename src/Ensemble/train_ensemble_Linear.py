...
from model.combineLinear import prepare_CombineLinear, prepare_LinearSVM
...
methods = ['CLM', 'MLM','RescoreBert_MD', 'RescoreBert_MWER',  'RescoreBert_MWED', 'Bert_sem', 'BertAlsem', 'PBERT', 'PBert_LSTM'] # , 'Bert_alsem', , 
wer_weight = {
    'ASR': 7.34,
    'CLM': 6.05,
    'MLM': 5.17,
...
train_dataset = prepare_ensemble_dataset(train_score_dict)
valid_dataset = prepare_ensemble_dataset(valid_score_dict)
...
if (train_args['svm']):
    model = prepare_LinearSVM(train_feature_num)
else:
    model = prepare_CombineLinear(train_feature_num, bce = train_args['BCE'] ,reduction = train_args['reduction'])
...
if (train_args['svm']):
    model_name = f"SVM_C{train_args['C']}"
else:
    model_name = "Linear"
...
for e in range(train_args['epoch']):
    ...
    for i, data in enumerate(tqdm(train_loader)):
        ...
        feature_rank = data['feature_rank'].to(device)
        # print(f'feature.shape:{feature.shape}')
        # print(f'feature:{feature}')
        
        # if (i > 100):
        #     exit(0)
        # feature = feature + feature_rank
        # print(f'feature_rank:{feature_rank}')
        ...
        if (train_args['use_rank']):
            ...
            loss = model(feature, nBestIndex, labels = labels)['loss']
    ...
    with torch.no_grad():
        for j, data in enumerate(tqdm(valid_loader)):
            ...
            if (train_args['use_rank']):
                ...
                output = model(feature, nBestIndex, labels = labels)
            loss = output['loss']
            logit = output['logit']
