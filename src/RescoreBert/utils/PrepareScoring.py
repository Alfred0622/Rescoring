...
def prepare_score_dict(data_json, nbest):
    ...
    if (isinstance(data_json, list)):
        ...
        for i, data in enumerate(data_json):
            ...
            legal_length = len(am_scores[-1]) if isinstance(am_scores[-1], list) else am_scores[-1].shape[0]

            lm_scores.append(
               data['lm_score'][:nbest]
                if ('lm_score' in data.keys() and data['lm_score'] != None and len(data['lm_score']) > 0) else [
            ...
            ctc_scores.append(
                ...
                if 'ctc_score' in data.keys() else [0.0 for _ in range(legal_length)]
                # np.zeros(
                #     am_scores[-1].shape[0], dtype = np.float32
                # )
            )
        ...
        for i, key in enumerate(data_json.keys()):
            ...
            lm_scores.append(
                ...
                if ('lm_score' in data_json[key].keys() and data_json[key]['lm_score'] != None and len(data_json[key]['lm_score']) > 0) else np.zeros(
                    legal_length, dtype = np.float32
                )
            )

            ctc_scores.append(
               data_json[key]['ctc_score'][:nbest]
                if ('lm_score' in data_json[key].keys() and data_json[key]['ctc_score'] != None and len(data_json[key]['ctc_score']) > 0) else np.zeros(
                    legal_length, dtype = np.float32
                )
            )
            ...
            hyps.append(
                data_json[key]['hyps'][:nbest]
            )

            refs.append(data_json[key]['ref'])
        
    am_scores = np.array(list(zip_longest(*am_scores, fillvalue=np.NINF)), dtype = np.float32)
...
def get_result(
    ...
    for utt, index in enumerate(max_index):
        ...
        ref_rescore = rescores[utt][hyps[utt].index(refs[utt])].item() if (refs[utt] in hyps[utt]) else "Not in candidates"
        ref_total = total_score[utt][hyps[utt].index(refs[utt])].item() if (refs[utt] in hyps[utt]) else "Not in candidates"
        ...
        if (index_before_min is not None):
            ...
            score_before_min = [total_score[utt][prev].item() for prev in index_before_min]
