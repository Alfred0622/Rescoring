...
setting = ['noLM', 'withLM']
for s in setting:
    with open(f'/mnt/disk6/Alfred/Rescoring/data/{dataset}/data/{s}/train/data.json', 'r') as f:
        print(f'json_load')
        json_data = json.load(f)
        total_keys = len(json_data)
        print(f'total_keys:{total_keys}')
        sep_key = int(total_keys / split_num)
        ...
        for i, data in enumerate(json_data):
            ...
            if (counter < split_num):
                if (i >= sep_key * counter):
                    new_path = Path(f'/mnt/disk6/Alfred/Rescoring/data/{dataset}/data/{s}/train_{counter}')
