...
if (len(sys.argv) != 2 and len(sys.argv) != 3):
    assert(len(sys.argv) == 2), "python ./train_RescoreBert.py {MD,MWER,MWED} checkpoint_Path(Optional)"
...
wandb.init(project = f"Rescore_{args['dataset']}_{setting}", config = wandb_config, name = f"RescoreBert_{mode}_batch{train_args['train_batch']}_accum{train_args['accumgrad']}grads_lr{train_args['lr']}")

optimizer.zero_grad(set_to_none = True)

checkpoint_path = Path(f"./checkpoint/{args['dataset']}/RescoreBert/{setting}/{mode}/{args['nbest']}best/batch{train_args['train_batch']}_accum{train_args['accumgrad']}grads_lr{train_args['lr']}")
...
for e in range(start_epoch, train_args['epoch']):
    ...
    checkpoint = {
        "epoch": e,
        "bert": model.bert.state_dict() if torch.cuda.device_count() == 1 else  model.module.bert.state_dict(),
        "fc": model.linear.state_dict() if torch.cuda.device_count() == 1 else  model.module.linear.state_dict(),
